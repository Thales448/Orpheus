...
class MarketHoursChecker:
    ...
    def time_until_market_open(self) -> Optional[timedelta]:
        ...
        max_days_ahead = 10  # Safety limit to prevent infinite loops
        days_checked = 0
        ...
        while days_checked < max_days_ahead:
            ...
            month_day = (next_date.month, next_date.day)
            market_holidays = [
                (1, 1),   # New Year's Day
                (7, 4),   # Independence Day
                (12, 25), # Christmas
            ...
            if month_day not in market_holidays:
...
class LiveStockIngestService:
    ...
    def parse_trade_message(self, message: str) -> Optional[Dict]:
        ...
        try:
            ...
            if isinstance(data, dict):
                ...
                if not hasattr(self, '_last_logged_header') or message_count <= 5:
                ...
                if header_type == 'TRADE' and 'trade' in data:
                    ...
                    if not self.dev_mode and self.symbols and root not in self.symbols:
                        message_count = getattr(self, '_message_count', 0)
                        if not hasattr(self, '_last_wrong_symbol') or message_count % 100 == 0:
    ...
    def calculate_timestamp(self, date: int, ms_of_day: int) -> Optional[datetime]:
        ...
        Returns timezone-aware datetime in US/Eastern timezone (market timezone)
        """
        try:
            # Parse date
            date_str = str(date)
            ...
            # Create base datetime at midnight in US/Eastern timezone
            et_tz = pytz.timezone('US/Eastern')
            base_time = et_tz.localize(datetime(year, month, day))
    ...
    async def handle_message(self, message: str):
        ...
        try:
            ...
            if trade:
                ...
                logger.info(
                    f"QUOTE {symbol} price={price_str} size={trade.get('size')} "
        ...
        while self.running:
            try:
                ...
                async with websockets.connect(
                    ...
                    # Do this BEFORE subscribing to avoid confusion
